...
pub mod pallet {
	...
	pub enum Error<T> {
		...
		// Request price info failed.
		RequestPriceError,

		// Parse price info failed.
		ParsePriceError,
	}
	...
	impl<T: Config> Pallet<T> {
		...
		fn append_or_replace_price(price: (u64, Permill)) {
			Prices::<T>::mutate(|prices| {
				...
				log::debug!("Prices vector: {:?}", prices);
			});
		}

		fn request_price() -> Result<Vec<u8>, rt_offchain::http::Error> {
			let deadline =
				sp_io::offchain::timestamp().add(rt_offchain::Duration::from_millis(2_000));
			let request =
				rt_offchain::http::Request::get("https://api.coincap.io/v2/assets/polkadot");
			let pending = request
				.deadline(deadline)
				.send()
		...
		fn parse_price(price: Vec<u8>) -> Option<(u64, Permill)> {
			let mut position = 0;
			for item in price.iter() {
				if *item == b'.' {
					break;
				}
				position += 1;
			}
			log::debug!("Parse price, position: {:?}", position);
			...
			log::debug!("Parse price, integer_str: {:?}", integer_str);

			let fraction_str = str::from_utf8(fraction).unwrap();
			log::debug!("Parse price, fraction_str: {:?}", fraction_str);

			let price_integer: u64 = integer_str.parse().unwrap();
			log::debug!("Parse price, price_integer: {:?}", price_integer);

			let price_fraction = Permill::from_parts(fraction_str.parse().unwrap());
			log::debug!("Parse price, price_fraction: {:?}", price_fraction);
		...
		fn fetch_price_info(block_number: T::BlockNumber) -> Result<(), Error<T>> {
			...
			let price = Self::request_price().map_err(|_| {
				log::warn!("Err happend when request price.");
				<Error<T>>::RequestPriceError
			})?;
			log::debug!("Price before parse: {}", str::from_utf8(&price.clone()).unwrap());

			let price = Self::parse_price(price).ok_or_else(|| {
				log::warn!("Parse price error!");
				<Error<T>>::ParsePriceError
			})?;
			log::debug!("Price parse : ({:?}, {:?})", price.0, price.1);

			// 使用无签名交易带有签名payload的方法，理由如下：
			// 1）DOT价格信息为公开信息，与特定帐户无关，采用无签名交易避免手续费；
			// 2）对上链数据签名，确保其来源能被验证。
			let result = Signer::<T, T::AuthorityId>::any_account().send_unsigned_transaction(
				|account| PricePayload { block_number, public: account.public.clone(), price },
				Call::submit_price_unsigned_with_signed_payload,
			);

			if let Some((_, res)) = result {
				return res.map_err(|_| {
					log::warn!("Failed in offchain_unsigned_tx_signed_payload");
